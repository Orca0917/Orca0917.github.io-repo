...
두 아이템 또는 유저간의 유사도를 측정하는 방법에 대해서 알아보자. 유사성을 측정하는 방법에는 여러가지가 있지만, 일반적으로 **거리의 역수 개념**을 사용한다. 거리가 멀수록 유사도는 작아지고, 거리가 가까울수록 유사도가 높아진다. 여기서 중요한 것은 **거리를 어떻게 측정하는지에 따라서 유사도의 값이 달라지는 것**이다.
