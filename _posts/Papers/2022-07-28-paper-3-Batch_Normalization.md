...
하지만, 이렇게 정규화를 진행하는 것은 분명히 좋지만 반대로 문제점도 있습니다. 바로 레이어가 표현할 수 있는 구간을 바꾸는 문제점이 있습니다. 예를 들어, 시그모이드 함수를 생각해봅시다. 시그모이드는 비선형 함수임에도, 특정 구간에서는 선형성을 띄기 때문에 정규화에 의해 선형성을 띄게 만들어질 수 있습니다. 이것은 딥러닝의 존재자체가 무의미해지는 결과가 됩니다.
